...
1. **Ingests TxLINE** consensus odds for live World Cup and International Friendlies fixtures (the free devnet World Cup tier).
...
4. **Commits and positions:** on a confirmed steam it SHA-256-hashes the full decision and anchors that commitment on Solana (a memo) BEFORE the match resolves, then opens a paper position sized by conviction against a 100-unit bankroll.
...
6. **Anchors the result** on Solana too, so the whole track record (call and outcome) is tamper-evident.
...
- **Honest metric:** graded on closing-line value and real result, the only edge metrics that survive contact with reality.

## TxLINE usage (primary input, end-to-end)
- `fixtures/snapshot` · the World Cup and friendlies board
- `odds/snapshot/{id}` · consensus 1X2, de-vigged via `Pct`
- `odds/snapshot/{id}?asOf=kickoff` · the exact closing line for true CLV
- `scores/snapshot/{id}` · match state and result for settlement
- `odds/validation` and on-chain `txoracle.validate_odds` · trustless proof the odds are canonical vs the on-chain daily root

## Architecture
- `signal.js` · pure, deterministic de-vig and steam scoring engine (unit-testable)
